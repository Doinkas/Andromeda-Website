...
export async function listAdminMatches({ teamId, limit = 50, upcomingOnly = true } = {}) {
  ...
  const teamConstraint = scopedTeamId
    ? where('teamId', '==', scopedTeamId)
    : where('teamId', 'in', TEAM_IDS);
  const q = query(matchesRef, teamConstraint);
  const snapshot = await getDocs(q);
  const now = Date.now();
  return snapshot.docs
    .map((docSnap) => ({ id: docSnap.id, ...docSnap.data() }))
    ...
    .sort((left, right) => {
      const leftDate = left.scheduledAt?.toDate ? left.scheduledAt.toDate() : new Date(left.scheduledAt);
      const rightDate = right.scheduledAt?.toDate ? right.scheduledAt.toDate() : new Date(right.scheduledAt);
    ...
    .slice(0, Math.min(Math.max(Number(limit) || 50, 1), 300));
}
...
export async function saveAdminMatch(match, performedByEmail = null) {
  ...
  const existing = match.id ? await getDoc(matchDoc) : null;
  if (existing?.exists()) {
...
export async function saveAdminMatchReport(matchId, report = {}) {
  const normalizedMatchId = normalizeString(matchId);
  if (!normalizedMatchId) {
    throw new Error('Match ID is required for result reporting.');
  }
  ...
  const authz = await requireAnyTeamPermission(['matches:write', 'matches:report'], teamId, {
    message: 'You are not authorized to report results for this team.'
  });

  await setDoc(matchRef, {
    result: normalizeResult(report.result),
    score: normalizeString(report.score) || null,
    mapScoreFor: normalizeNumber(report.mapScoreFor),
    mapScoreAgainst: normalizeNumber(report.mapScoreAgainst),
    mapsPlayed: normalizeStringList(report.mapsPlayed),
    replayCode: normalizeString(report.replayCode) || null,
    notes: normalizeString(report.notes) || null,
