...
export const ALL_APPLICATION_PERMISSIONS = Object.freeze(
  Object.values(STAFF_PERMISSIONS).filter((permission) => (
    permission !== STAFF_PERMISSIONS.TEAMS_ASSIGNED
  ))
);

const ROLE_PERMISSION_MAP = Object.freeze({
  superadmin: ALL_APPLICATION_PERMISSIONS,
  owner: Object.freeze([
    ...
    STAFF_PERMISSIONS.MATCHES_REPORT,
...
export function normalizeStaffRecord(record = {}, fallback = {}) {
  ...
  return {
    ...
    nameSetupComplete: record?.nameSetupComplete === true
      ? true
      : (record?.nameSetupComplete === false ? false : null)
  };
}
...
export function resolveClaimRole(claims = {}) {
  ...
  const roles = unique([directRole, ...parseRoleList(claims.roles)]);
  const priority = ['superadmin', 'owner', 'admin', 'manager', 'media', 'captain'];
  const resolvedRole = priority.find((role) => roles.includes(role));
  if (resolvedRole) return resolvedRole;

  return claims.captain === true ? 'captain' : null;
}
...
export function resolveStaffAccessSnapshot({
  ...
  let nameSetupComplete = null;
  let staffRecordName = null;

  if (staffRecordExists) {
    const staffProfile = normalizeStaffRecord(staffRecord, { uid, email, name });
    staffRecordName = String(staffRecord?.name || '').trim();
    ...
    nameSetupComplete = staffProfile.nameSetupComplete;
  } else if (allowlisted) {
    role = 'admin';
...
export function authzHasRole(authz = {}, allowedRoles = []) {
  ...
  if (authzIsSuperAdmin(authz)) return true;
  return Boolean(authz?.isAuthorized && roles.includes(normalizeStaffRole(authz.role)));
}

export function authzIsSuperAdmin(authz = {}) {
  return Boolean(
    authz?.isAuthorized
    && normalizeStaffRole(authz?.role) === 'superadmin'
  );
}
...
export function authzHasAnyPermission(authz = {}, permissions = []) {
  ...
  if (
    authzIsSuperAdmin(authz)
    && normalizedPermissions.some((permission) => ALL_APPLICATION_PERMISSIONS.includes(permission))
  ) return true;
...
export function authzHasAllPermissions(authz = {}, permissions = []) {
  ...
  if (
    authzIsSuperAdmin(authz)
    && normalizedPermissions.every((permission) => ALL_APPLICATION_PERMISSIONS.includes(permission))
  ) return true;
  const granted = Array.isArray(authz?.permissions) ? authz.permissions : [];
  return Boolean(
    authz?.isAuthorized
