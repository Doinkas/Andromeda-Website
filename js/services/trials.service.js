...
import { applyTrialsReadPlan, buildTrialsReadPlan } from './trials-query.js';
...
export async function listTrials(filters = {}) {
  ...
  const readPlan = buildTrialsReadPlan(authz, filters);
  // Rules are not filters, so each query must prove an authorized team scope.
  // Status filtering and sorting stay local to avoid compound-index requirements.
  const teamClause = readPlan.scopedTeamId
    ? where('teamId', '==', readPlan.scopedTeamId)
    : where('teamId', 'in', readPlan.teamIds);
  const trialsQuery = query(trialsCollection, teamClause);
  const snapshot = await getDocs(trialsQuery);
  const trials = snapshot.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data() }));
  return applyTrialsReadPlan(trials, readPlan);
}
...
export async function setTrialStatus(trialId, status, performedByEmail = null) {
  ...
  const performedBy = String(performedByEmail || '').trim().toLowerCase() || null;
  const transitionData = { status };

  if (status === 'dropped') {
    transitionData.droppedAt = serverTimestamp();
    transitionData.droppedBy = performedBy;
  } else if (status === 'rejected') {
    transitionData.rejectedAt = serverTimestamp();
    transitionData.rejectedBy = performedBy;
  }

  await updateTrial(trialId, transitionData, performedBy);
}
