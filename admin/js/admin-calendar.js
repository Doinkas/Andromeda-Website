import {
  ...
  upsertMatches
} from '/js/services/matches.service.js';
...
function logCalendarError(operation, collectionName, error, teamId = '') {
  console.error('Calendar operation failed:', {
    operation,
    collection: collectionName,
    code: error?.code || null,
    message: error?.message || String(error),
    teamId: String(teamId || '').trim().toLowerCase() || null,
    role: adminRole,
    error
  });
}
...
function toggleModalType(type) {
  ...
  if (type === 'event') {
    ...
    modalTeam.removeAttribute('required');
    ...
    modalTeam.setAttribute('required', '');
    modalOpponent.setAttribute('required', '');
  }
}

function setMatchEditorMode({ type = 'match', isNew = false } = {}) {
  const reportOnly = type === 'match' && !canManageMatches && canReportMatches;
  ...
  modalType.disabled = reportOnly;
  modalTeam.disabled = reportOnly || Boolean(assignedTeamId);
  scheduleFields.forEach((field) => {
    if (field) field.disabled = reportOnly;
  });
  if (modalDelete) {
    modalDelete.hidden = isNew || reportOnly || (type === 'event' && !canManageEvents);
  }
  if (modalSave) {
    modalSave.textContent = reportOnly ? 'Save Result' : 'Save';
  }
}
...
async function handleImportSubmit(event) {
  ...
  try {
    ...
    logCalendarError('import matches', 'matches', error, currentTeamFilter);
    setStatus(importStatus, 'Could not import matches. Check the data and try again.', 'error');
  } finally {
...
async function loadMatches() {
  try {
    ...
    logCalendarError('load matches', 'matches', error, currentTeamFilter);
    allMatches = [];
    return error;
  }
}
...
async function loadEvents() {
  try {
    ...
    logCalendarError('load events', 'events', error);
    allEvents = [];
    return error;
  }
}
...
function renderSelectedDayDetail() {
  ...
  matches.forEach((match) => {
    ...
    edit.textContent = canManageMatches ? 'Edit' : 'Update Result';
    edit.addEventListener('click', () => openMatchModal(match));

    item.appendChild(info);
    if (canManageMatches || canReportMatches) item.appendChild(edit);
...
function renderCalendarDay(date, isCurrentMonth) {
  ...
  if (isSelected) {
    ...
    if (canManageMatches) actionsEl.appendChild(matchButton);
...
async function openMatchModal(match) {
  ...
  setMatchEditorMode({ type: 'match', isNew: false });
  openModal();
}
...
async function openEventModal(event) {
  ...
  setMatchEditorMode({ type: 'event', isNew: false });
  openModal();
}

function createNewMatchForDay(date) {
  if (!canManageMatches) {
    setStatus(calendarStatus, 'Your role cannot create scheduled matches.', 'error');
    return;
  }
  ...
  setMatchEditorMode({ type: 'match', isNew: true });
  openModal();
...
function createNewEventForDay(date) {
  ...
  setMatchEditorMode({ type: 'event', isNew: true });
  openModal();
  selectedDateKey = null;
  renderCalendar();
}
...
async function handleMatchFormSubmit(event) {
  ...
  try {
    ...
    if (type === 'match') {
      ...
      const reportOnly = !canManageMatches && canReportMatches;
      ...
      if (reportOnly && !matchId) {
        setStatus(modalStatus, 'Captains can report results for existing matches only.', 'error');
        return;
      }
      if (!reportOnly) {
      ...
      const reportPayload = {
        result: matchPayload.result,
        score: matchPayload.score,
        mapScoreFor: matchPayload.mapScoreFor,
        mapScoreAgainst: matchPayload.mapScoreAgainst,
        mapsPlayed: matchPayload.mapsPlayed,
        replayCode: matchPayload.replayCode,
        notes: matchPayload.notes,
        screenshotUrls
      };

      const savedMatchId = reportOnly
        ? await saveAdminMatchReport(matchId, reportPayload)
        : await saveAdminMatch(matchPayload, adminEmail);
      ...
      if (pendingScreenshotFiles.length) {
        ...
        updateScreenshotNote(activeMatchScreenshotUrls);
        if (reportOnly) {
          await saveAdminMatchReport(savedMatchId, { ...reportPayload, screenshotUrls });
        } else {
      ...
      const existingEventId = modalMatchId.value.trim();
      const eventId = existingEventId || `event_${Date.now()}`;
    ...
    logCalendarError(
      `save ${type}`,
      type === 'event' ? 'events' : 'matches',
      error,
      type === 'match' ? modalTeam.value : ''
    );
    setStatus(modalStatus, `Could not save ${type}. Please try again.`, 'error');
  } finally {
...
async function handleMatchDelete() {
  ...
  if ((type === 'match' && !canManageMatches) || (type === 'event' && !canManageEvents)) {
    setStatus(modalStatus, `Your role cannot delete this ${type}.`, 'error');
    return;
  }
  ...
  try {
    ...
    setStatus(modalStatus, `Could not delete ${type}. Please try again.`, 'error');
  } finally {
...
window.addEventListener('admin:authorized', async (event) => {
  ...
  adminUid = String(event?.detail?.user?.uid || '').trim() || null;
  adminName = String(event?.detail?.name || '').trim() || adminEmail;
  adminRole = String(event?.detail?.role || '').trim().toLowerCase() || null;
  canManageMatches = authzHasAnyPermission(event?.detail, ['matches:write']);
  canReportMatches = authzHasAnyPermission(event?.detail, ['matches:report']);
  canManageEvents = authzHasAnyPermission(event?.detail, ['events:write']);
  const canManageAllTeams = authzHasAnyPermission(event?.detail, ['teams:any']);
